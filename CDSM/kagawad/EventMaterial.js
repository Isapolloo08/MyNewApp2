...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, FlatList, Button, Alert, Platform } from 'react-native';
import Icon from 'react-native-vector-icons/FontAwesome';

const EventMaterial = ({ navigation, route }) => {
  const { eventName, startDate, endDate, location, note, selectedCouncilor } = route.params;
  ...
  const handleNextPress = () => {
    if (materialsList.length === 0) {
      ...
      navigation.navigate('OtherExpenses', {
        eventName,
        startDate,
        endDate,
        location,
        note,
        selectedCouncilor, // Passing councilor to next screen
  ...
  return (
    <FlatList
      data={[{ key: 'header' }, { key: 'content' }, { key: 'footer' }]}
      renderItem={({ item }) => {
        switch (item.key) {
          case 'header':
            return (
