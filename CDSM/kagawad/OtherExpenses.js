...
const OtherExpenses = ({ navigation, route }) => {
  const { eventName, startDate, endDate, location, note, selectedCouncilor, savedMaterialsList } = route.params;
  const [expenseName, setExpenseName] = useState('');
  const [quantity, setQuantity] = useState('');
  const [pricePerUnit, setPricePerUnit] = useState(''); // Renamed to pricePerUnit
  ...
  const addExpense = () => {
    if (expenseName.trim() && quantity.trim() && pricePerUnit.trim()) {
      const newExpense = {
        name: expenseName,
        quantity: parseInt(quantity),
        pricePerUnit: parseFloat(pricePerUnit), // Changed to pricePerUnit
        allocation: parseInt(quantity) * parseFloat(pricePerUnit), // Calculation uses pricePerUnit
      };
      ...
      setExpenseName('');
      setQuantity('');
      setPricePerUnit(''); // Reset pricePerUnit
    } else {
  ...
  const handleItemPress = (index) => {
    if (index === selectedIndex) {
      ...
      setPricePerUnit(expensesList[index].pricePerUnit.toString()); // Use pricePerUnit
    }
  };
